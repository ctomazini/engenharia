...
engenharia.dashboard.filters = {
	...
	render(container, data, page) {
		...
		const buttons = this.period_options()
			.map(
				(op) => `
			<button type="button" class="eng-dash-periodo-btn${current === op.days ? " active" : ""}"
				data-period-days="${op.days}">${frappe.utils.escape_html(op.label)}</button>`
			)
			.join("");
		...
		container.html(`
			<div class="eng-dash-periodo-bar">
				<div class="eng-dash-filtro-group">
					...
					<div class="eng-dash-periodo-filters">${buttons}</div>
				</div>
			</div>
		`);
	},
